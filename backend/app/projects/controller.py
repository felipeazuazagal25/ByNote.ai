...
async def create_project_route(project: ProjectCreate, db: AsyncSession = Depends(get_db), user: User = Depends(current_active_user)):
    db_project = await create_project(project, db, user)
    return db_project
